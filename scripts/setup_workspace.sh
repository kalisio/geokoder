...
if [ "$CI" = true ]; then
    ...
    # NOTE: cloning geokoder could be avoided if we could parse app_version from tag/branch name instead
    # In this case, the kli would clone geokoder
    ...
    git clone --depth 1 $GIT_OPS "$GITHUB_URL/kalisio/geokoder.git" "$WORKSPACE_DIR/geokoder"
